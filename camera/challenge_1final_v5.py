...
ERROR_DISTANCE = 10
ROTATE_ANGLE = 0.30
SAFE_DIST = 50  # RoboRad/2 + ball radius ori:43
...
###final control
cha1_stage = 1
x_nextpoint_index = 45 #原45
y_target_index = 30 #原30
x_target_index = 60 #原60
...
def strategy():
    ...
    if cha1_stage == 1:
        ...
        #適合一直線運動之位置
        print("Cha1_stage==", cha1_stage)
        if block[1] + ROB_RANG <= ball_p[1] <= block[2] - ROB_RANG and ball_p[0] > 300 and stopit == 0:
            # i = (900 - ball_p[0]) // 60
...
def get_sent_cmd(sentcmd, update):
    ...
    if update:
        ...
        if sentcmd[0][0] == 'N' and stage == 4:
            ...
            global kick_flag
            kick_flag = True
        if sentcmd[1][0] == 'j' or sentcmd[1][0] == 'h':
            global kicked
            # print("DONE")
            kicked = True
