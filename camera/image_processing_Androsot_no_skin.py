...
CAMERA_DISTANCE = 1 #相機在底線後1.2 meter #5/24
FIELD_LENGTH = 3.6
...
TEAM_COLOR = "red"   #本隊隊伍顏色 #5/25
NUM_OF_BLUE_TEAM = 3
NUM_OF_RED_TEAM = 3
SIDE = 1  #踢球方向 朝我們踢 1 朝俄羅斯佬踢 -1 #5/25
        ...
        if(index != 999):#,3,5,7,9,counterclockwise
            ...
            if SIDE == 1:
                if self.camera_setting == "left":
                    # offset
                    self.x_left = round(self.aruco_x_list[index] - (CAMERA_DISTANCE*100 ) ,2) -10
                    self.y_left = round((FIELD_WIDTH * 100 // 2 + (self.aruco_y_list[index] )  ) ,2) +20
                    self.x_left = self.x_left + 0.1 * (130 -self.y_left )
                    self.y_left = self.y_left + 0.1 * (130 -self.y_left )
                    ...
                    self.y_right = round((FIELD_WIDTH * 100 // 2 - (self.aruco_y_list[index] ) ),2) -30

                    self.y_right = self.y_right + 0.1 * (130 -self.y_right )
            ...
            if SIDE == -1:
                if self.camera_setting == "left": 
                    # offset
                    self.x_left = round(self.aruco_x_list[index] - (CAMERA_DISTANCE*100 ) ,2) - 35
                    self.y_left = round((FIELD_WIDTH * 100 // 2 + (self.aruco_y_list[index] )  ) ,2) +20 
                    ...
                    self.x_right = round(FIELD_LENGTH* 100 - (self.aruco_x_list[index] - (CAMERA_DISTANCE*100 ) ),2)  + 10
                    self.y_right = round((FIELD_WIDTH * 100 // 2 - (self.aruco_y_list[index] ) ),2) -20
...
class Ball():
    ...
    def update_position(self, frame, camera_setting):
        ...
        # 膚色遮罩
        mask_skin = colorMasking(frame, "skin", color_selection_setting='custom', camera_selection_setting=camera_setting)

        # 去掉膚色
        mask_no_skin = cv2.bitwise_and(mask, cv2.bitwise_not(mask_skin))

        # 顯示去膚色後的遮罩
        #cv2.imshow("mask_no_skin", mask_no_skin)
        
        contours, _ = cv2.findContours(mask_no_skin, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
        ...
        for cnt in contours:
            ...
            #print("check",w,h)
            #1300 2500
            if w * h > 700 and w * h < 4500 and w < 3 * h and h < 3 * w:  # 過濾面積小於150的矩形
                ...
                if 50 > radius > 15:
                    ...
                    text = f"r={int(radius)}"  # 顯示半徑（取整數）
                    cv2.putText(frame, text, (x, y - 5), font, 0.7, (0,255,0), 2)  # 在圖上標出半徑
                    datacenter.append(center)  # 儲存球心座標
                    dataradius.append(radius)  # 儲存半徑
...
def image_result():
    ...
    while True:
        for camera in camera_list:
            ...
            # #5/24
            # if camera.index == "left" and SIDE == 1 :
            #     frame = cv2.rotate(frame, cv2.ROTATE_180)

            # if camera.index == "right" and SIDE == -1 :
            #     frame = cv2.rotate(frame, cv2.ROTATE_180)
...
#5/14
    ...
    ############################################################################################# draw
            ...
            if challenge3.kick_goal != [0, 0]:  # kick_goal
                arri_pos_in_frame = change_coordinate_for_draw([challenge3.kick_goal])
                draw_the_point(frame, arri_pos_in_frame, "goal", [challenge3.kick_goal])
...
def write_to_json(color,lower_hsv,upper_hsv,camera_selection_setting):
    if camera_selection_setting == "left":
        file_path = 'color_settings_left_restrict.json'
    elif camera_selection_setting == "right":
        file_path = 'color_settings_right_restrict.json'
